...
Static build generated by `uv run whereabouts-build-pwa`, deployed to Cloudflare Pages (project `whereabouts-app`, served at `whereabouts.adamdent.uk`) with `npx wrangler pages deploy docs/ --project-name whereabouts-app`. The old `whereabouts-app.pages.dev` URL 301s to the custom domain via a Pages Functions middleware at `functions/_middleware.js`.
...
The PWA replaced the sideload/TestFlight plan: users install from the URL. Live at whereabouts.adamdent.uk; tested on real iOS/Android devices, and by Colin Day himself. Feedback loop ongoing.
...
- **Hosting.** Resolved: static PWA on Cloudflare Pages (project `whereabouts-app`) at whereabouts.adamdent.uk. Incremental deploys, CDN cache headers via `_headers`, no server. The retired `whereabouts-app.pages.dev` host 301s to the custom domain via `functions/_middleware.js`. Deploy is a double-click (`Publish App Update.command`) using the local wrangler session, or by hand with `wrangler pages deploy docs/`.
...
- **Analytics (implemented 2026-07-29).** Self-hosted [Counterscale](https://counterscale.dev) (open source, cookieless, no consent banner) on its own Cloudflare Worker + Analytics Engine, replacing GoatCounter. The tracker (`docs/counterscale.min.js`, the vendored ESM module build; wired in `pwa.py` via `_analytics_snippet`) fires one pageview on load for referrers plus a manual pageview per map open with a virtual path `/village/<district>/<village>`, so usage aggregates to village level. It never logs house-level searches or what a visitor typed, honouring the standing caution that a log of searched houses can be sensitive. Site id `whereabouts`; one instance can serve the other portfolio sites via distinct site ids.
